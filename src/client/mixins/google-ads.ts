export class GoogleAdsAware {
  filterGoogleAdsURLs(requests, aid, group, atag) {
    const filteredUrl = requests.filter(r => r.url.startsWith('https://ad.doubleclick.net') || r.url.startsWith('https://fls.doubleclick.net')).map(request => decodeURIComponent(request.url));
