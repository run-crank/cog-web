...
export class CheckGoogleAnalyticsPageView extends BaseStep implements StepInterface {
  ...
  async executeStep(step: Step): Promise<RunStepResponse> {
    ...
    try {
      const requests = await this.client.getFinishedGoogleAnalyticsRequests();
      const urls = requests.filter(r => r.method == 'GET').map(r => r.url);
      const actual = urls.filter(url => url.includes('https://www.google-analytics.com')
                                        && url.includes('/collect')
                                        && url.includes('t=pageview')
                                        && url.includes(`tid=${id}`));
      const params = this.getUrlParameter(decodeURIComponent(actual[0]));
      if (actual.length !== 1) {
